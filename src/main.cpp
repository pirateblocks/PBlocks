...
#if defined(NDEBUG)
# error "PirateBlocks cannot be compiled without assertions."
#endif
...
unsigned int nStakeMinAge = 60 * 20; // 20 min
...
const string strMessageMagic = "PirateBlocks Signed Message:\n";
...
int64_t GetProofOfWorkReward(int64_t nFees)
{
	...
	if(pindexBest->nHeight+1 > 0 && pindexBest->nHeight+1 < 11)
    {
		nSubsidy = 7700000 * COIN;
    } 
...
int64_t GetProofOfStakeReward(int64_t nCoinAge, int64_t nFees)
{
	int64_t nRewardCoinYear = COIN_YEAR_REWARD;
	 ...
	 if(pindexBest->nHeight > 100000 && pindexBest->nHeight < 200000)
    {
        nRewardCoinYear = COIN_YEAR_REWARD / 2;
    }
    else if(pindexBest->nHeight > 199999 && pindexBest->nHeight < 300000)
    {
        nRewardCoinYear = COIN_YEAR_REWARD / 4;
    }
    else if(pindexBest->nHeight > 299999 )
    {
        nRewardCoinYear = COIN_YEAR_REWARD2;
    }	
...
#ifdef ENABLE_WALLET
// PirateBlocks: attempt to generate suitable proof-of-stake
...
{
    RenameThread("PirateBlocks-loadblk");
