...
{
    // PirateBlocks: check prefix
    if(uri.scheme() != QString("PirateBlocks"))
        return false;
...
{
    // Convert PirateBlocks:// to PirateBlocks:
    //
    //    Cannot handle this later, because bitcoin:// will cause Qt to see the part after // as host,
    //    which will lower-case it (and thus invalidate the address).
    if(uri.startsWith("PirateBlocks://"))
    {
        uri.replace(0, 12, "PirateBlocks:");
    }
...
{
    return GetSpecialFolderPath(CSIDL_STARTUP) / "PirateBlocks.lnk";
}
...
{
    return GetAutostartDir() / "PirateBlocks.desktop";
}
...
HelpMessageBox::HelpMessageBox(QWidget *parent) :
    QMessageBox(parent)
{
    header = tr("PirateBlocks-Qt") + " " + tr("version") + " " +
        QString::fromStdString(FormatFullVersion()) + "\n\n" +
        tr("Usage:") + "\n" +
        "  PirateBlocks-qt [" + tr("command-line options") + "]                     " + "\n";
    ...
    setWindowTitle(tr("PirateBlocks-Qt"));
